...
try{
  ...
  //name input
   ...
   $nameerror="Name is required";$run=1;
   	 ...
   	 $name = test_input($_POST["name"]);$name=strtoupper($name);
...
if(!in_array($file_type,$allowed)) {
  $imgerror="Only jpg, png, and jpeg files are allowed.";$run=1;
  }
  else{
...
$sql="insert into patient_detail_entry(aadhar_id,p_name,phone_no,father_name,gender,date_of_birth,house_no,town_vill,address1,district,state,security_code,email_id,pincode,patient_pic)"."values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
   if($stmt=mysqli_prepare($con,$sql)){
 mysqli_stmt_bind_param($stmt,"isissssssssssis",$Aadhar,$name,$phone,$fathername,$gender,$date,$house,$town,$address,$district,$state,$enc_password,$email,$pincode,$img);
