...
if(empty($_POST["primaryid"])){
  ...
  if(empty($_POST["datarecord"])){
     ...
     //refer or not to hq
     $refer="";
 $refer=referverify($_POST["ref"]);
  if($refer==""){
  	$refererror="Please choose a value";$run=1;
  }else{
...
$sql="insert into district_record(aadhar_id,doctor_id,disease_name,remark,primary_center_id,number_of_recorded_data,surgery,operation_remark,operation_id,reffered_headquarter)"."values(?,?,?,?,?,?,?,?,?,?)";
   if($stmt=mysqli_prepare($con,$sql)){ 
 mysqli_stmt_bind_param($stmt,"iisssissss",$aadhar,$doctorid,$disease,$remark,$primaryid,$datanum,$operate,$operationfor,$operationremark,$refer);echo "bind";
...
<h1>DISTRICT RECORD</h1>
	...
	<div id="b7">
		...
		<div id="b20">
		<b>REFFERED HEADQUARTER</b>	
			<div >
		<b>YES</b>
			<input type="radio" name="ref" value="yes">
			<b>NO</b>
			<input type="radio" name="ref" value="no"><span>*</span><br>
			<?php echo "$refererror"; ?>
		</div>
	</div>
		<div id="b8">
