<?php
session_start();
if(!isset($_SESSION["flag"]) && !($_SESSION["position"]=="DISTRICT_SURGERY")){
...
$dbName= "healthcare";
//initialize errors
$aadharerror=$doctoriderror=$primaryiderror=$headverifyerror=$remarkerror=$opiderror=$refererror=$verifyerror=$operationerror="";$run=0;
...
if($run==1)
echo "verify".$patientverify."this";
//operation id
...
$doctorid=$_SESSION["id"];
	...
	//forward to hq
	$headverify=referverify(test_input($_POST["head"]));
if($headverify==""){
	$headverifyerror="Please select value";$run=1;
}
echo $headverify;
...
$sql="insert into district_surgery(aadhar_id,operation_remark,operation_verify,operation_doctor_id,forward_to_headquarter,operation_id)"."values(?,?,?,?,?,?)";
   if($stmt=mysqli_prepare($con,$sql)){ 
 mysqli_stmt_bind_param($stmt,"ississ",$aadhar,$remark,$patientverify,$doctorid,$headverify,$operationid);echo "bind";
